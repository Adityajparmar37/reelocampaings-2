...
export default function CampaignsPage() {
  ...
  const { items, meta, loading, launchLoading, error } = useSelector(
    (s) => s.campaigns,
  );
  ...
  // Listen to campaign status updates
  useEffect(() => {
    const handleStatusUpdate = (data) => {
      console.log('[CampaignsPage] Status update:', data)
      dispatch(setCampaignStatus({ campaignId: data.campaignId, status: data.status }))
  ...
  const handleCreate = async (e) => {
    ...
    const res = await dispatch(
      createCampaign({ ...form, audienceFilters: { tags } }),
    );
    if (!res.error) {
      setModal(false);
      ...
      <div className="table-container">
        <table className="table">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={7} className="td text-center py-16 text-gray-600">
              ...
              items.map((c) => {
                const pct =
                  c.totalRecipients > 0
                    ? (((c.sentCount || 0) / c.totalRecipients) * 100).toFixed(
                        1,
                      )
                    : null;
                return (
                  <tr
                    ...
                    <td className="td">
                      <Link
                        to={`/campaigns/${c._id}`}
                        className="text-white font-medium hover:text-brand-400 transition-colors block">
                        {c.name}
                      </Link>
                      <p className="text-xs text-gray-600 mt-0.5 max-w-xs truncate">
                        {c.messageTemplate?.slice(0, 55)}…
                      </p>
                    </td>
                    <td className="td">
                      <span
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-lg shadow-2xl animate-slide-up">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <input
                  className="input"
                  placeholder="vip, newsletter, active"
                ...
                <p className="text-xs text-gray-600 mt-1">
                  Leave empty to target all contacts
                </p>
              </div>
